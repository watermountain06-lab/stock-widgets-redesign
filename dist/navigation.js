...
window.bindCardNavigation = function(stage, move) {
  ...
  const reset=()=>{const el=card();if(el){el.style.transform='';el.style.willChange='';}gesture=null;};
  stage.addEventListener('pointerdown', event => {
    if (!['touch','pen'].includes(event.pointerType) || event.isPrimary===false || event.target.closest('input,select,textarea,summary,[contenteditable]')) return;
    reset();
  ...
  stage.addEventListener('pointermove', event => {
    ...
    const el=card();if(el){el.style.willChange='transform';el.style.transform='translateX('+Math.max(-120,Math.min(120,dx*.65))+'px)';}
  });
